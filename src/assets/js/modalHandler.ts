...
class ModalHandler {
  ...
  static mixerPanFunc(angle: number, id: string) {
    const circumference = knobFactory.getCircumference();
    const slotPanDom = document.getElementById(`outerRingslotPan${id}`);
    if (slotPanDom != null) {
      slotPanDom.setAttribute(
        'stroke-dashoffset',
      ...
      console.error(`outerRingslotPan${id} does not exist!`);
    }
  }

  mixerColumnClicked(number: number) {
    if (this.lastColumnClicked != null) {
      this.lastColumnClicked.classList.remove('mixerColumnActive');
    }
    const newEl = document.getElementById(`mixerColumn${number}`);
    newEl?.classList.add('mixerColumnActive');
    this.lastColumnClicked = newEl;
  }

  getMixerVolumeContext(i: number) {
    return this.mixerVolumeContexts[i];
  }

  createMixerColumn(number: number, name: string) {
    const mixerColumn = document.createElement('div');
    mixerColumn.setAttribute('id', `mixerColumn${number}`);
    mixerColumn.setAttribute('class', 'mixerColumn');
    // mixerColumn.style.background = sequencer.getBackgroundColor(number);
    const slotNumber = document.createElement('div');
    slotNumber.setAttribute('class', 'slotNumber unselectable');
    slotNumber.textContent = number.toString();
    const slotName = document.createElement('div');
    slotName.setAttribute('class', 'slotName unselectable');
    slotName.textContent = name;
    const slotVolume = document.createElement('canvas');
    slotVolume.setAttribute('class', 'slotVolume');
    slotVolume.setAttribute('width', '20');
    slotVolume.setAttribute('height', '82');
    slotVolume.setAttribute('id', `slotVolume${number}`);
    this.mixerVolumeContexts[number] = slotVolume.getContext('2d')!;
    const slotActive = document.createElement('div');
    slotActive.setAttribute('class', 'slotActive');
    slotActive.addEventListener('click', () => {
      const drumInfo = audioEngine.noteToDrum.get(35 + number);
      if (drumInfo != null) {
        if (drumInfo[4]) {
          slotActive.classList.add('slotInActive');
        } else {
          slotActive.classList.remove('slotInActive');
        }
        drumInfo[4] = !drumInfo[4];
      }
    });
    ...
    const panRange = {
      start: ((drumInfo[3] + 1) / 2) * 127,
      min: 0,
      max: 127,
    };
    const panKnob = knobFactory.createKnob(`slotPan${number}`, number, ModalHandler.mixerPanFunc, panRange, true);
    slotPan.appendChild(panKnob);
    ...
    mixerColumn.appendChild(slotVolume);
    mixerColumn.appendChild(slotActive);
    mixerColumn.appendChild(slotPan);
    mixerColumn.appendChild(slotSlider);
    mixerColumn.addEventListener('click', () => {
      this.mixerColumnClicked(number);
    });
    return mixerColumn;
  }
  ...
  createMixer() {
    ...
    for (const drumInfo of audioEngine.noteToDrum.values()) {
      const name = drumInfo[0];
      const mixerColumn = this.createMixerColumn(number, name);
      mixerMain.appendChild(mixerColumn);
      number += 1;
    }
  }
